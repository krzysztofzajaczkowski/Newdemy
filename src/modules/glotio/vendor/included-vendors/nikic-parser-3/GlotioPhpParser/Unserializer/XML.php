...
namespace GlotioPhpParser\Unserializer;

use DomainException;
use GlotioPhpParser\Unserializer;
use XMLReader;
...
class XML implements Unserializer
{
    ...
    protected function readComment() {
        $className = $this->reader->getAttribute('isDocComment') === 'true'
            ? 'GlotioPhpParser\Comment\Doc'
            : 'GlotioPhpParser\Comment'
        ;
    ...
    protected function getClassNameFromType($type) {
        $className = 'GlotioPhpParser\\Node\\' . strtr($type, '_', '\\');
