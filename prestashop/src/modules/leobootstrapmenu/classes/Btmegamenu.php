    ...
    public function genTree($parent, $level)
    {
        if ($this->hasChild($parent)) {
            ...
            foreach ($data as $menu) {
                ...
                <div><span class="disclose"><span></span></span>'.($menu['title'] ? $menu['title'] : '').' (ID:'.$menu['id_btmegamenu'].') <input type="checkbox" name="menubox[]" value="'.$menu['id_btmegamenu'].'" class="quickselect" title="Select to delete"><span title="'. $this->leo_module->l('Edit').'" class="quickedit" rel="id_'.$menu['id_btmegamenu'].'">E</span><span title="'. $this->leo_module->l('Delete').'" class="quickdel" rel="id_'.$menu['id_btmegamenu'].'">D</span><span title="'. $this->leo_module->l('Duplicate').'" class="quickduplicate" rel="id_'.$menu['id_btmegamenu'].'">DUP</span></div>';
                if ($menu['id_btmegamenu'] != $parent) {
    ...
    public function genFrontTree($parent_id, $level, $parent, $typesub = '', $group_type = '')
    {
        ...
        if ($this->hasChild($parent_id)) {
            ...
            if ($parent['colums'] > 1) {
                ...
                if ($parent['sub_with'] == 'submenu') {
                    $output = '<div class="'.$class.' level'.$level.'" ><div class="dropdown-menu-inner">';
